...
(function() {
    ...
    async function loadAllResponses() {
        ...
        if (allResponses.length > 0) {
            ...
            userResponse = {
                answered: lastResponse.answered || false,
                accepted: lastResponse.accepted || false,
                date: lastResponse.date || null,
                time: lastResponse.time || null,
                location: lastResponse.location || null,
                timestamp: lastResponse.timestamp || null
            };
    ...
    function updateAdminPanel() {
        ...
        if (allResponses.length === 0) {
            adminResponse.textContent = '⏳ هنوز پاسخی ثبت نشده';
        ...
        if (last.accepted && last.date && last.time) {
            adminResponse.textContent = '✅ قبول کرده!';
            adminResponse.className = 'admin-value accepted';
            adminDate.textContent = last.date || '-';
            adminTime.textContent = last.time || '-';
            adminLocation.textContent = last.location || '-';
            adminTimestamp.textContent = last.timestamp || '-';
        } else if (last.answered && !last.accepted) {
